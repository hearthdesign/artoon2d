...
# Import the User model for the author field
from django.contrib.auth.models import User
# Import TaggableManager for tagging functionality
    ...
    def toggle_follow(self, target_profile):
        if Follow.objects.filter(from_profile=self, to_profile=target_profile).exists():
            Follow.objects.filter(
                from_profile=self, to_profile=target_profile).delete()
            return 'unfollowed'
        else:
...
class Follow(models.Model):
    from_profile = models.ForeignKey(
        'Profile', related_name='following_relations', on_delete=models.CASCADE)
    to_profile = models.ForeignKey(
        'Profile', related_name='follower_relations', on_delete=models.CASCADE)
...
class Post(models.Model):
    ...
    author = models.ForeignKey(
        User, on_delete=models.CASCADE, editable=False, related_name='posts')
